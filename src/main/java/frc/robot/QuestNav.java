...
public class QuestNav {
  ...
  /**
   * Resets the robot's position to the specified coordinates
   * 
   * @param x The desired X coordinate
   * @param y The desired Y coordinate
   */
  public void resetPoseTo(Pose2d pose) {
    Translation2d rawTranslation = getQuestNavPose().getTranslation();
    Translation2d desiredPos = pose.getTranslation();

    // Calculate the offset such that the adjusted pose will be at the desired
    // position
    Translation2d offset = rawTranslation.minus(desiredPos);
    resetPosition = new Pose2d(offset, resetPosition.getRotation());

    // Send the reset command to the Quest headset
    if (questMiso.get() != 99) {
      questMosi.set(1);
    }
  }
